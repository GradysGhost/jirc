plugins.dice = {
        ...
        if (matches) {
          ...
          if (dice <= 100 && sides <= 100) {
            if (dice >= 1 && sides >= 2) {
              ...
              say(client, channel,
			    'Sorry, but you need to roll at least 1 die with at least 2 sides.'
		      );
            }
          } else {
            say(client, channel,
			  'Sorry, I can roll no more than 100 dice with no more than 100 sides.'
			);
          }
        } else {
          say(client, channel,
		    'Command must be in the form of: ' + config.commandChar +
			'dice <dice>d<sides>'
	      );
        }
      }
  ...
  help : function() {
    return [
      '    ' + config.commandChar +
	  'dice <count>d<sides> -- Roll <count> dice with <sides> sides each.'
    ];
  }
