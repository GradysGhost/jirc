plugins.logbrowser = {
  ...
  logChanged : function(event, filename) {
    ...
    if (stats.size >= plugins.logbrowser.config.maxLogBytes) {
      ...
      // Close out the live file, move it,
	  // reopen the stream, start watching that file
      stream.end();
      ...
      fs.watch(newStream.path,
	    {'persistent':true}, plugins.logbrowser.logChanged
      );
    }
  },
  ...
  init : function() {
    ...
    plugins.logbrowser.config = JSON.parse(
	  fs.readFileSync('plugins/logbrowser.json', 'utf8')
	);
    ...
    for (var i in plugins.logger.streams) {
      fs.watch(plugins.logger.streams[i].path,
	    {'persistent':true}, plugins.logbrowser.logChanged
      );
    }
    ...
    require('http').createServer(function onCreate(request, response) {
      ...
      var rbod = fs.readFileSync(
	    plugins.logbrowser.config.docroot + '/' +
		plugins.logbrowser.config.prologue, 'utf8'
	  );
      ...
      q.a = ((typeof q.a !== 'undefined')
	    ? parseInt(decodeURIComponent(q.a))
		: 0
	  );
      q.b = ((typeof q.b !== 'undefined')
	    ? parseInt(decodeURIComponent(q.b))
		: 0
	  );
      q.p = ((typeof q.p !== 'undefined')
	    ? parseInt(decodeURIComponent(q.p))
		: 0
	  );
      ...
      if (q.chan) { // If a channel was given...
        // ...and if the requested channel exists...
        if (fs.existsSync('./log/' + q.chan)) {
          // Some heading stuff
          rbod += '<h2>Channel log: ' + q.chan + '</h2>';
          rbod += '<form action=\'/\' method=\'GET\'>'
		    + '<p>Search ' + q.chan
			+ ': <input type=\'hidden\' name=\'chan\' value=\''
			+ q.chan + '\' /><input type=\'search\' name=\'q\' size=\'15\' value=\''
			+ q.q + '\' /> ';
          rbod += 'Lines before: <input type=\'text\' name=\'b\' value=\''
		    + q.b
			+ '\' size=\'3\' /> Lines after: <input type=\'text\' name=\'a\' value=\''
			+ q.a + '\' size=\'3\' /> <input type=\'submit\' value=\'Search\' />'
			+ '</form></p>';
          if (q.q) {  // Are we attempting a search? If yes...
            ...
            while (fs.existsSync('./log/' + q.chan + '.' + i)) {
              var nextLines = fs.readFileSync(
			    './log/' + q.chan + '.' + i, 'utf8').split('\n'
			  );
            ...
            for (var i = 0; i < lines.length; ++i) {
              ...
              if (m) {
                ...
                rbod += e.htmlEncode(lines[i]).replace(
				  regex, '<span class=\'querymatch\'>' + q.q + '</span>'
				) + '\n';
                if (q.a) {
                  rbod += '</span>';
            ...
            if (q.p <= lim && q.p != 1) {
              rbod += '<a href=\'/?chan=' + encodeURIComponent(q.chan)
			    + '&p=' + prev + '\'>&laquo;</a>';
            }
            if (q.p === 0) {
              rbod += 'Page ' + (lim + 1);
            ...
            rbod += '<pre>' + e.htmlEncode(fs.readFileSync(
			  './log/' + file, 'utf8'
			)) + '</pre>';
          }
        ...
        if (q.q) {  // Does the user want to run a global search?
          ...
          rbod += '<h3>Search: ' + q.q + '</h3>';
          rbod += '<form action=\'/\' method=\'GET\'>'
		    + '<p>Search all channels:'
			+ '<input type=\'search\' name=\'q\' size=\'15\' value=\'' + q.q + '\' /> ';
          rbod += 'Lines before: <input type=\'text\' name=\'b\' value=\''
		    + q.b
			+ '\' size=\'3\' /> Lines after: <input type=\'text\' name=\'a\' value=\''
			+ q.a + '\' size=\'3\' />'
        ...
        // Not doing a global search, and no channel provided,
		// so just list the channels
        } else {  
          rbod += '<h2>Channels</h2><ul>';
          rbod += '<form action=\'/\' method=\'GET\'>'
		    + '<p>Search all channels: <input type=\'search\' name=\'q\''
			+ 'size=\'15\' value=\'' + q.q + '\' /> ';
          rbod += 'Lines before: <input type=\'text\' name=\'b\' value=\''
		    + q.b + '\' size=\'3\' />'
			+ 'Lines after: <input type=\'text\' name=\'a\' value=\'' +
			+ q.a + '\' size=\'3\' />'
			+ '<input type=\'submit\' value=\'Search\' /></form></p>';
          for (var client in config.clients) {
            for (var channel in config.clients[client].options.channels) {
              rbod += '<li><a href=\'/?chan=' +
			    encodeURIComponent(config.clients[client].options.channels[channel])
				+ '\'>' + config.clients[client].options.channels[channel] + '</a></li>';
            }
          }
          rbod += '</ul>';
        }
      ...
      // Dat epilogue
      rbod += fs.readFileSync(
	    plugins.logbrowser.config.docroot + '/'
		+ plugins.logbrowser.config.epilogue, 'utf8'
      );

      response.writeHead(200,
	    {'content-type':'text/html', 'length':rbod.length}
	  );
      response.end(rbod);
